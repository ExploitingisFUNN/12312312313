...
-- // Add the set_enabled function
function espfunctions.set_enabled(instance, state)
    if not instance or not espinstances[instance] then return false end
    
    local data = espinstances[instance]
    
    if data.box then
        if data.box.outline then data.box.outline.Visible = state and esplib.box.enabled end
        if data.box.fill then data.box.fill.Visible = state and esplib.box.enabled end
        
        for _, line in ipairs(data.box.corner_outline) do
            line.Visible = state and esplib.box.enabled and esplib.box.type == "corner"
        end
    ...
    if data.healthbar then
        if data.healthbar.outline then data.healthbar.outline.Visible = state and esplib.healthbar.enabled end
        if data.healthbar.fill then data.healthbar.fill.Visible = state and esplib.healthbar.enabled end
    end
    
    if data.name then
        data.name.Visible = state and esplib.name.enabled
    end
    
    if data.distance then
        data.distance.Visible = state and esplib.distance.enabled
    end
    
    if data.tracer then
        if data.tracer.outline then data.tracer.outline.Visible = state and esplib.tracer.enabled end
        if data.tracer.fill then data.tracer.fill.Visible = state and esplib.tracer.enabled end
    end
...
-- // Make espinstances accessible
esplib.objects = espinstances
