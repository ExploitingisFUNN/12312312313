...
function Library:UpdateColorsUsingRegistry()
    ...
    for _, Effect in pairs(Library.GlowEffects) do
        if Effect.Instance and Effect.Instance.Parent then
            local Color = Effect.ColorFunc and Effect.ColorFunc() or Library.Scheme.AccentColor
            if Effect.Stroke and Effect.Stroke.Parent then
                Effect.Stroke.Color = Color
            end
            if Effect.OuterStroke and Effect.OuterStroke.Parent then
                Effect.OuterStroke.Color = Color
            end
        end
    end
end
...
function Library:AddGlowEffect(Frame: GuiObject, Thickness: number?, Color: any)
    local ColorVal = Color or Color3.fromRGB(255, 85, 0)
    local ColorFunc = typeof(Color) == "function" and Color or nil

    local OutermostGlow = New("UIStroke", {
        ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
        Color = ColorVal,
        Thickness = (Thickness or 25),
    ...
    New("UIGradient", {
        Transparency = NumberSequence.new({
            NumberSequenceKeypoint.new(0, 0.67),
            NumberSequenceKeypoint.new(0.3, 0.8),
            NumberSequenceKeypoint.new(0.8, 0.95),
            NumberSequenceKeypoint.new(1, 1)
        }),
        Parent = OutermostGlow
    })

    local OuterGlow = New("UIStroke", {
        ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
        Color = ColorVal,
    ...
    table.insert(Library.GlowEffects, {
        Instance = Frame,
        OutermostStroke = OutermostGlow,
        OuterStroke = OuterGlow,
        InnerStroke = InnerGlow,
        CoreStroke = CoreGlow,
        ColorFunc = ColorFunc,
    })

    return OutermostGlow, OuterGlow, InnerGlow, CoreGlow
end
    ...
    function Funcs:AddButton(...)
        ...
        local function CreateButton(Button)
            local Base = New("TextButton", {
                ...
                BackgroundTransparency = Button.Disabled and 0.2 or 0.05,
                Size = UDim2.fromScale(1, 1),
                Text = Button.Text,
                TextSize = 14,
                TextTransparency = Button.Disabled and 0.7 or 0.3,
            ...
            local Stroke = New("UIStroke", {
                Color = "OutlineColor",
                Transparency = Button.Disabled and 0.7 or 0.3,
                Parent = Base,
            })
        ...
        local function InitEvents(Button)
            Button.Base.MouseEnter:Connect(function()
                ...
                Button.StrokeTween = TweenService:Create(Button.Stroke, Library.TabTweenInfo, {
                    Transparency = 0.1,
                })
        ...
        function Button:AddButton(...)
            ...
            function SubButton:UpdateColors()
                ...
                StopTween(SubButton.StrokeTween)

                SubButton.Base.BackgroundColor3 = SubButton.Disabled and Library.Scheme.BackgroundColor
                    or Library.Scheme.MainColor
                SubButton.Base.BackgroundTransparency = SubButton.Disabled and 0.2 or 0.05
                SubButton.Base.TextTransparency = SubButton.Disabled and 0.7 or 0.3
                SubButton.Stroke.Transparency = SubButton.Disabled and 0.7 or 0.3
        ...
        function Button:UpdateColors()
            ...
            Button.Base.BackgroundColor3 = Button.Disabled and Library.Scheme.BackgroundColor
                or Library.Scheme.MainColor
            Button.Base.BackgroundTransparency = Button.Disabled and 0.2 or 0.05
            Button.Base.TextTransparency = Button.Disabled and 0.7 or 0.3
            Button.Stroke.Transparency = Button.Disabled and 0.7 or 0.3
...
function Library:CreateWindow(WindowInfo)
    ...
    local windowWidth = WindowInfo.Width or WindowInfo.Size.X.Offset
    local windowHeight = WindowInfo.Height or WindowInfo.Size.Y.Offset
    
    WindowInfo.Size = UDim2.fromOffset(
        math.clamp(windowWidth, Library.MinSize.X, MaxX),
        math.clamp(windowHeight, Library.MinSize.Y, MaxY)
    )
    ...
    Library.OriginalWindowSize = WindowInfo.Size

    
    do
        local scriptVersion = "N/A"
        pcall(function()
            scriptVersion = game:HttpGet("https://api.exploitingis.fun/version.txt") or "N/A"
            scriptVersion = tostring(scriptVersion):gsub("\n", ""):gsub("\r", "")
        end)

        local baseFooter = WindowInfo.Footer or ""
        if baseFooter ~= "" then
            WindowInfo.Footer = string.format("%s | %s | exploitingis.fun", baseFooter, scriptVersion)
        else
            WindowInfo.Footer = string.format("%s | exploitingis.fun", scriptVersion)
        end
    end
    ...
    do
        ...
        local function CreateHamburgerLine(yPos)
            return New("Frame", {
                BackgroundColor3 = "FontColor",
                Position = UDim2.new(0, 6, 0, yPos),
                Size = UDim2.new(1, -12, 0, 2),
                Parent = SidebarToggleButton,
            })
        end

        local HamburgerLine1 = CreateHamburgerLine(8)
        local HamburgerLine2 = CreateHamburgerLine(15)
        local HamburgerLine3 = CreateHamburgerLine(22)
        ...
        local function ToggleSidebar()
            ...
            local TabsTargetSize, ContainerTargetSize, ContainerTargetPosition
            if SidebarVisible then
                
                TabsTargetSize = UDim2.new(0.3, 0, 1, -70)
                ContainerTargetSize = UDim2.new(0.7, -1, 1, -70)
                ContainerTargetPosition = UDim2.new(1, 0, 0, 49)
            else
                
                TabsTargetSize = UDim2.new(0, 0, 1, -70)
                ContainerTargetSize = UDim2.new(1, -1, 1, -70)
                ContainerTargetPosition = UDim2.new(1, 0, 0, 49)
            end

            
            local TabsTween = TweenService:Create(Tabs, ToggleTweenInfo, {Size = TabsTargetSize})
            local ContainerSizeTween = TweenService:Create(Container, ToggleTweenInfo, {Size = ContainerTargetSize})
            local ContainerPosTween = TweenService:Create(Container, ToggleTweenInfo, {Position = ContainerTargetPosition})
            
            TabsTween:Play()
            ContainerSizeTween:Play()
            ContainerPosTween:Play()

            
            if SidebarVisible then
                
                TweenService:Create(HamburgerLine1, ToggleTweenInfo, {
                    Rotation = 0,
                    Position = UDim2.new(0, 6, 0, 8),
                    Size = UDim2.new(1, -12, 0, 2)
                }):Play()
                TweenService:Create(HamburgerLine2, ToggleTweenInfo, {
                    Rotation = 0,
                    Position = UDim2.new(0, 6, 0, 15),
                    Size = UDim2.new(1, -12, 0, 2),
                    BackgroundTransparency = 0
                }):Play()
                TweenService:Create(HamburgerLine3, ToggleTweenInfo, {
                    Rotation = 0,
            ...
            ContainerSizeTween.Completed:Connect(function()
                for _, Tab in pairs(Library.Tabs) do
                    if Tab.Resize then
                        Tab:Resize(true)
                    end
        ...
        SidebarToggleButton.MouseButton1Click:Connect(ToggleSidebar)

        
        SidebarToggleButton.MouseEnter:Connect(function()
            TweenService:Create(SidebarToggleButton, Library.TweenInfo, {
    ...
    function Window:AddTab(Name: string, Icon)
        ...
        do
            ...
            if Icon then
                ...
                Library:AddGlowEffect(TabIcon, 4)
            ...
            TabLeft = New("ScrollingFrame", {
                ...
                ScrollBarImageColor3 = function()
                    return Library:GetBetterColor(Library.Scheme.AccentColor, -20)
                end,
                Parent = TabContainer,
            })
            New("UICorner", {
        ...
        function Tab:Resize(ResizeWarningBox: boolean?)
            ...
            local function CalculateSideMinWidth(Side)
                local minWidth = 200 * Library.DPIScale 
                local maxTextWidth = minWidth
                            ...
                            if descendant:IsA("TextLabel") or descendant:IsA("TextButton") then
                                if descendant.Visible and descendant.Text ~= "" then
                                    local textWidth, _ = Library:GetTextBounds(
                                        descendant.Text,
                                        descendant.FontFace or Library.Scheme.Font,
                                        descendant.TextSize,
                                        math.huge 
                                    )
                                    
                                    local totalWidth = textWidth + 40 * Library.DPIScale
                                    if totalWidth > maxTextWidth then
            ...
            local availableWidth = TabContainer.AbsoluteSize.X
            
            if availableWidth > 0 then
                local leftMinWidth = CalculateSideMinWidth(TabLeft)
                local rightMinWidth = CalculateSideMinWidth(TabRight)
                local totalMinWidth = leftMinWidth + rightMinWidth + 6
                
                if Library.ActiveTab == Tab then
                    local containerPadding = 12 * Library.DPIScale
                    local sidebarWidth = MainFrame and (MainFrame.Size.X.Offset * 0.3) or 0
                    local requiredWindowWidth = totalMinWidth + containerPadding + sidebarWidth + 50 * Library.DPIScale 
                    local originalWindowWidth = Library.OriginalWindowSize and Library.OriginalWindowSize.X.Offset or 720
                    
                    if MainFrame and WindowInfo.Resizable then
                        if requiredWindowWidth > MainFrame.Size.X.Offset then
                            local newWidth = math.min(requiredWindowWidth, workspace.CurrentCamera.ViewportSize.X - 64)
                            if newWidth > MainFrame.Size.X.Offset then
                                MainFrame.Size = UDim2.fromOffset(newWidth, MainFrame.Size.Y.Offset)
                                availableWidth = TabContainer.AbsoluteSize.X
                            end
                        elseif requiredWindowWidth <= originalWindowWidth and MainFrame.Size.X.Offset > originalWindowWidth then
                            MainFrame.Size = UDim2.fromOffset(originalWindowWidth, MainFrame.Size.Y.Offset)
                            availableWidth = TabContainer.AbsoluteSize.X
                        end
                    end
                end
                ...
                if totalMinWidth <= availableWidth then
                    local remainingWidth = availableWidth - 6
                    local leftRatio = leftMinWidth / (leftMinWidth + rightMinWidth)
                    leftWidth = math.max(leftMinWidth, remainingWidth * leftRatio)
                    rightWidth = remainingWidth - leftWidth
                else
                    if leftMinWidth > rightMinWidth then
                        leftWidth = math.min(leftMinWidth, availableWidth * 0.7)
                        rightWidth = availableWidth - leftWidth - 6
                    else
                        rightWidth = math.min(rightMinWidth, availableWidth * 0.7)
                        leftWidth = availableWidth - rightWidth - 6
                    end
                end
                
                leftWidth = math.max(leftWidth, 150 * Library.DPIScale)
                rightWidth = math.max(rightWidth, 150 * Library.DPIScale)
                
                TabLeft.Position = UDim2.new(0, 0, 0, Offset)
                TabLeft.Size = UDim2.new(0, math.floor(leftWidth), 1, -Offset)
                
                local rightStartPos = leftWidth + 6
                TabRight.Position = UDim2.new(0, rightStartPos + math.floor(rightWidth), 0, Offset)
                TabRight.Size = UDim2.new(0, math.floor(rightWidth), 1, -Offset)
        ...
        function Tab:AddGroupbox(Info)
            ...
            do
                ...
                New("UIStroke", {
                    ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
                    Color = function()
                        return Library:GetBetterColor(Library.Scheme.OutlineColor, 8)
                    end,
                    Thickness = 1,
                    Transparency = 0.3,
                    Parent = GroupboxHolder,
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:AddTab(Name)
                ...
                function Tab:Resize()
                    ...
                    if Tab.Tab and Tab.Tab.Resize then
                        Tab.Tab:Resize()
                    end
                end
        ...
        function Tab:Hover(Hovering)
            ...
            TweenService:Create(TabButton, Library.TabTweenInfo, {
                ...
                Size = Hovering and UDim2.new(1, 0, 1, -1) or UDim2.new(1, 0, 1, -2),
            }):Play()
    ...
    function Window:AddKeyTab(Name)
        ...
        function Tab:AddKeyBox(...)
            ...
            Button.MouseEnter:Connect(function()
                TweenService:Create(Button, Library.TabTweenInfo, {
                    BackgroundTransparency = 0,
                    TextTransparency = 0,
                }):Play()
            end)
    ...
    function Window:SetSidebarVisible(visible: boolean)
        if SidebarVisible ~= visible then
            ToggleSidebar()
        end
    end

    function Window:IsSidebarVisible()
